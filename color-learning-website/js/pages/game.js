...
function updateColorPreview(){const color=getCurrentRgb();colorPreview.style.background=rgbString(color);}

function bindPair(rangeInput,numberInput,onDragEnd){const min=Number(rangeInput.min),max=Number(rangeInput.max);rangeInput.addEventListener('input',()=>{numberInput.value=rangeInput.value;updateColorPreview();});rangeInput.addEventListener('change',()=>{if(onDragEnd)onDragEnd();});numberInput.addEventListener('input',()=>{numberInput.value=String(clampRange(numberInput.value,min,max));rangeInput.value=numberInput.value;updateColorPreview();});numberInput.addEventListener('change',()=>{pushRecentColor(getCurrentRgb());});}
...
function bindModelControls(){bindPair(rgbInputs.r,rgbInputs.rn,()=>pushRecentColor(getCurrentRgb()));bindPair(rgbInputs.g,rgbInputs.gn,()=>pushRecentColor(getCurrentRgb()));bindPair(rgbInputs.b,rgbInputs.bn,()=>pushRecentColor(getCurrentRgb()));bindPair(hsvInputs.h,hsvInputs.hn,()=>pushRecentColor(getCurrentRgb()));bindPair(hsvInputs.s,hsvInputs.sn,()=>pushRecentColor(getCurrentRgb()));bindPair(hsvInputs.v,hsvInputs.vn,()=>pushRecentColor(getCurrentRgb()));bindPair(cmykInputs.c,cmykInputs.cn,()=>pushRecentColor(getCurrentRgb()));bindPair(cmykInputs.m,cmykInputs.mn,()=>pushRecentColor(getCurrentRgb()));bindPair(cmykInputs.y,cmykInputs.yn,()=>pushRecentColor(getCurrentRgb()));bindPair(cmykInputs.k,cmykInputs.kn,()=>pushRecentColor(getCurrentRgb()));colorModelSelect.addEventListener('change',()=>{setVisibleModelGroup(colorModelSelect.value);updateColorPreview();pushRecentColor(getCurrentRgb());});}
